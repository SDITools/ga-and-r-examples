...
pacman::p_load(googleAnalyticsR,  # How we actually get the Google Analytics data
               ...
               caret)             # For k-fold cross-validation
...
This is a simple data pull. We're just pulling data by day (because each day is a different day of the week) and sessions.
...
head(ga_data) %>% kable() %>% kable_styling()
...
We need to assign the day of week to each day of traffic and then convert each day of week to a binary -- or "dummy" -- variable. This will make sense as we get into it.
...
kable(head(ga_data_dummies)) %>% kable_styling()
...
# We don't actually want the "date" value, so we're doing to drop that
...
ind_vars <- model_summary$coefficients %>% as.data.frame() %>% 
  ...
  mutate(Significance = ifelse(`Pr(>|t|)` < 0.01, "99%", 
                               ifelse(`Pr(>|t|)` < 0.05, "95%",
                                      ifelse(`Pr(>|t|)` < 0.1, "90%", 
                                             ifelse(`Pr(>|t|)` < 0.2, "80%","<80%"))))) %>% 
  dplyr::select(Variable, Coefficient = Estimate, Significance)

# Output a table with the independent variables
kable(ind_vars) %>% kable_styling()
...
# Output the head
kable(head(predict_vs_actual_df)) %>% kable_styling()
